...
class UserInfoController(Resource):
    ...
    def get(uid=None):
        ...
        try:
            user_service = UserService()
            current_user = user_service.get_user_by_id(user_id=user_id)
...
class UserPwdController(Resource):
    ...
    def patch(uid=None):
        ...
        try:
            ...
            if not user:
                return jsonify(status_code.USER_NOT_EXIST)
            if not user_service.check_user_password(user_id=uid, password=data.get("old_password")):
                return jsonify(status_code.PASSWORD_ERROR)
            user_service.update_user_password(user_id=uid, new_password=data.get("new_password"))
            return jsonify(status_code.SUCCESS)
        except Exception as ex:
            print(ex)
            return jsonify(status_code.FAIL)
